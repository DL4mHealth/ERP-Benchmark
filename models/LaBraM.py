...
def load_labram_student(model, ckpt_path, strict=False, map_location="cpu"):
    ckpt = torch.load(ckpt_path, map_location=map_location)
    raw = ckpt.get("model", ckpt)

    # only load the student part of the checkpoint
    sd = {k[len("student."):]: v for k, v in raw.items() if k.startswith("student.")}

    msd = model.state_dict()
    for name in ["head.weight", "head.bias"]:
        if name in sd and name in msd and sd[name].shape != msd[name].shape:
            sd.pop(name)
...
"""if __name__ == '__main__':
    ...
    model_path = '../checkpoints/LaBraM/pretrain_labram/LaBraM/labram-base.pth'

    load_labram_student(model, model_path)
    model.reset_classifier(2)

    # weight only loading
    pretrained_state_dict = torch.load(model_path, map_location='cpu', weights_only=True)
    ...
    x = torch.randn(2, 128, 8, 200, device=device)  # [B, N, A, T]
    y = model(x)
    print(y.shape)"""
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.channel_mapping = nn.Conv1d(
            ...
            out_channels=128,  # default 128 in LaBraM checkpoint
            kernel_size=1,
            padding='same',
            bias=False,
        )
        ...
        # since the author of LaBraM does not use swa, we directly load the model here for simplicity
        if os.path.exists(model_path) and configs.is_training == 1 and configs.task_name == "supervised":
            missing_keys, unexpected_keys = load_labram_student(self.model, model_path,
    ...
    def supervised(self, x_enc, x_mark_enc):  # x_enc (batch_size, seq_length, enc_in)
        ...
        enc_out = self.channel_mapping(x_enc.permute(0, 2, 1))  # (batch_size, 128, seq_length)
        # half-overlap patches
        enc_out = enc_out.unfold(dimension=2, size=200, step=100)  # (batch_size, 128, num_patches, patch_size)
        # enc_out = enc_out.reshape(x_enc.shape[0], 128, -1, 200)  # (batch_size, 128, num_patches, patch_size)
