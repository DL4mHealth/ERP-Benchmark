...
def get_id_list_rlpd(args, label_path, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for RLPD data
    ...
    for filename in os.listdir(label_path):
        ...
        # [subject_id, disease_id] For RLPD, subject ID and disease ID should be the same for all samples a subject
        subject_id_disease_id = subject_label[0, 1:3]
...
class RLPDLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        a, b = 0.6, 0.8
        self.all_ids, self.train_ids, self.val_ids, self.test_ids = get_id_list_rlpd(args, self.label_path, a, b)
        ...
        self.y = self.y[:, [2, 1]]  # only keep [disease_id, subject_id] for RLPD
