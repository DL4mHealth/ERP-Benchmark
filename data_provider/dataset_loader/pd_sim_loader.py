...
def get_id_list_pd_sim(args, label_path, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for PD-SIM data
    ...
    for filename in os.listdir(label_path):
        ...
        # [subject_id, disease_id] For PD-SIM, subject ID and disease ID should be the same for all samples a subject
    ...
    # [task_id, congruent_condition, accuracy, subject_id, disease_id]
...
class PDSIMLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        a, b = 0.6, 0.8
        self.all_ids, self.train_ids, self.val_ids, self.test_ids = get_id_list_pd_sim(args, self.label_path, a, b)
        ...
        self.y = self.y[:, [4, 3]]  # only keep [disease_id, subject_id] for PD-SIM
