...
def get_id_list_adhd_wmri(args, label_path, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for ADHD-WMRI data
    ...
    adhd_list = list(data_list[np.where(data_list[:, 1] == 1)][:, 0])  # Parkinson's disease IDs
    if args.cross_val == 'fixed' or args.cross_val == 'mccv':  # fixed split or Monte Carlo cross-validation
        ...
        random.shuffle(adhd_list)

        train_ids = hc_list[:int(a * len(hc_list))] + adhd_list[:int(a * len(adhd_list))]
        val_ids = (hc_list[int(a * len(hc_list)):int(b * len(hc_list))] +
                   adhd_list[int(a * len(adhd_list)):int(b * len(adhd_list))])
        test_ids = hc_list[int(b * len(hc_list)):] + adhd_list[int(b * len(adhd_list)):]
...
class ADHDWMRILoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        a, b = 0.6, 0.8
        self.all_ids, self.train_ids, self.val_ids, self.test_ids = get_id_list_adhd_wmri(args, self.label_path, a, b)
