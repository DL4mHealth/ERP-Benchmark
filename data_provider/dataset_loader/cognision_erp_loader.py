...
def get_id_list_cognision_erp(args, label_path, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for Cognision-ERP data
    ...
    for filename in os.listdir(label_path):
        ...
        # [disease_id, stimulus_type, subject_id]
        # [subject_id, disease_id] For Cognision-ERP,
        # subject ID and disease ID should be the same for all samples a subject
        subject_id_disease_id = subject_label[0, [2, 0]]
    ...
    ad_list = list(data_list[np.where(data_list[:, 1] == 1)][:, 0])  # Alzheimer's disease IDs
...
class CognisionERPLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        a, b = 0.6, 0.8
        self.all_ids, self.train_ids, self.val_ids, self.test_ids = get_id_list_cognision_erp(args, self.label_path, a, b)
        ...
        self.y = self.y[:, [0, 2]]  # only keep [disease_id, subject_id] for Cognision-ERP
        # self.y = self.y[:, [1, 2]]  # only keep [stimulus_type, subject_id] for Cognision-ERP
        # self.y = self.y - np.array([1, 0])  # change stimulus_type from [1,2,3] to [0,1,2]
