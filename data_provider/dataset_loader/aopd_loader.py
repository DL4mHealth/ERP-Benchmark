...
def get_id_list_aopd(args, label_path, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for AOPD data
    ...
    for filename in os.listdir(label_path):
        ...
        # [task_id, stimulus_type, subject_id, disease_id]
...
class AOPDLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        a, b = 0.6, 0.8
        self.all_ids, self.train_ids, self.val_ids, self.test_ids = get_id_list_aopd(args, self.label_path, a, b)
        ...
        self.y = self.y[:, [3, 2]]  # only keep [disease_id, subject_id] for AOPD
        """self.y = self.y[:, [1, 2]]  # only keep [stimulus_type, subject_id] for AOPD
