...
def get_id_list_mtbi_odd(args, label_path, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for mTBI-ODD data
    ...
    for filename in os.listdir(label_path):
        ...
        # [subject_id, disease_id] For mTBI-ODD
        # subject ID and disease ID should be the same for all samples a subject
    ...
    mtbi_list = list(data_list[np.where(data_list[:, 1] == 0)][:, 0])  # mTBI
    hc_list = list(data_list[np.where(data_list[:, 1] == 1)][:, 0])  # Healthy Control
    ctbi_list = list(data_list[np.where(data_list[:, 1] == 2)][:, 0])  # Chronic TBI
    if args.cross_val == 'fixed' or args.cross_val == 'mccv':  # fixed split or Monte Carlo cross-validation
        ...
        random.shuffle(mtbi_list)
        random.shuffle(hc_list)
        random.shuffle(ctbi_list)

        train_ids = (mtbi_list[:int(a * len(mtbi_list))] + hc_list[:int(a * len(hc_list))]
                     + ctbi_list[:int(a * len(ctbi_list))])
        val_ids = (mtbi_list[int(a * len(mtbi_list)):int(b * len(mtbi_list))] +
                   hc_list[int(a * len(hc_list)):int(b * len(hc_list))] +
                   ctbi_list[int(a * len(ctbi_list)):int(b * len(ctbi_list))])
        test_ids = (mtbi_list[int(b * len(mtbi_list)):] + hc_list[int(b * len(hc_list)):]
                    + ctbi_list[int(b * len(ctbi_list)):])
...
class MTBIODDLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        a, b = 0.6, 0.8
        self.all_ids, self.train_ids, self.val_ids, self.test_ids = get_id_list_mtbi_odd(args, self.label_path, a, b)
        ...
        # self.y = self.y[:, [4, 2]]  # only keep [disease_id, subject_id] for mTBI-ODD
        self.y = self.y[:, [1, 2]]  # only keep [stimulus_type, subject_id] for mTBI-ODD
