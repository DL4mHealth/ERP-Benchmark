...
3. Epoch the data from **-2.0 s to +1.0 s** around the response event.
4. Baseline-correct the epochs using the interval **(-0.2 s to -0.0 s)**.
...
- **y**: Labels with shape `(N, 3)` in the format:
  `[task_id, subject_id, disease_id]`

Where:
- `task_id` = always 1 for Reinforcement learning
